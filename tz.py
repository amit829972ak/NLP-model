...
def load_spacy_model():
    try:
        return spacy.load("en_core_web_sm")  # Use small model instead of large model
    except OSError:
        st.warning("Model not found. Please install the model using: python -m spacy download en_core_web_sm")
        return None

nlp = load_spacy_model()
if nlp is None:
    st.error("Failed to load spaCy model. Please install the required model.")
    st.stop()
    ...
    with tab3:
        ...
        for acc in accommodations:
            with st.expander(acc.get("name", "Unnamed Accommodation")):
                ...
                with col2:
                    # Add booking functionality with direct link
                    booking_url = f"https://www.booking.com/search.html?ss={acc.get('name', '')}"
                    st.markdown(f"""
